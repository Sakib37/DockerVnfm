...
public class DockerVimCaller extends VimDriver {
  ...
  public boolean setEnvironmentVariable(
      VimInstance vimInstance, String containerID, List<String> envVariables) throws Exception {
    List<Serializable> params = new LinkedList<>();
    params.add(vimInstance);
    params.add(containerID);
    params.add((Serializable) envVariables);
    boolean res;
    try {
      res = (boolean) pluginCaller.executeRPC("setEnvironmentVariable", params, boolean.class);
    } catch (Exception e) {
  ...
  public boolean copyArchiveFromContainer(
    ...
    boolean res;
    try {
      res = (boolean) pluginCaller.executeRPC("copyArchiveFromContainer", params, boolean.class);
    } catch (IOException | PluginException | InterruptedException e) {
      throw new Exception(e.getMessage());
    }
    return res;
  ...
  public void restartServer(VimInstance vimInstance, String serverId) throws Exception {
    List<Serializable> params = new LinkedList<>();
    params.add(vimInstance);
    params.add(serverId);
    try {
